...
$query1="INSERT INTO `thesis`.`assettesting` (`statusID`, `PersonRequestedID`, `FloorAndRoomID`, `officeID`, `remarks`, `rpsmsrf`)
									VALUES ('8', '{$userID}', '{$FloorAndRoomID}', '{$officeID}', '{$remarks}', '{$rpsmsrf}');";
...
for($i = 0; $i<count($assets);$i++)
	{
		...
		$query4 = "UPDATE `thesis`.`asset` SET `status`='8' WHERE `assetID`='{$assets[$i]}';";
		$result4 = mysqli_query($dbc, $query4);
		//Insert to Audit table
		$query5 = "INSERT INTO `thesis`.`assetaudit` (`status`, `UserID`, `date`, `assetID`) VALUES ('8', '{$userID}', '{$date}', '{$assets[$i]}');";
		$result5 = mysqli_query($dbc, $query5);
	}
	
echo $query1."<br>";echo $query2."<br>";echo $query3."<br>";echo $query4."<br>";echo $query5."<br>";
